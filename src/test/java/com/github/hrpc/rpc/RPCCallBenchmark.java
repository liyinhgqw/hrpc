...
import com.github.hrpc.rpc.protobuf.TestProtos;
import com.github.hrpc.rpc.protobuf.TestRpcServiceProtos;
import com.github.hrpc.util.CommonConfigurationKeys;
import com.github.hrpc.util.Option;
import com.github.hrpc.rpc.MultithreadedTestUtil.TestContext;
...
public class RPCCallBenchmark {
  private Option conf = new Option();
  ...
  private Server startServer(MyOptions opts) throws IOException {
    ...
    if (opts.rpcEngine == ProtobufRpcEngine.class) {
      ...
      server = new RPC.Builder(conf).setProtocol(TestProtoBufRpc.TestRpcService.class)
      ...
      server = new RPC.Builder(conf).setProtocol(TestRPC.TestProtocol.class)
  ...
  public int run(String[] args) throws Exception {
    ...
    RPC.setProtocolEngine(conf, TestProtoBufRpc.TestRpcService.class, opts.rpcEngine);
  ...
  private TestContext setupClientTestContext(final MyOptions opts)
    ...
    for (int i = 0; i < numProxies; i++) {
      proxies[i] = createRpcClient(opts);
    }
  ...
  private RpcServiceWrapper createRpcClient(MyOptions opts) throws IOException {
    ...
    if (opts.rpcEngine == ProtobufRpcEngine.class) {
      final TestProtoBufRpc.TestRpcService proxy = RPC.getProxy(TestProtoBufRpc.TestRpcService.class, 0, addr, conf);
      ...
      final TestRPC.TestProtocol proxy = RPC.getProxy(
          TestRPC.TestProtocol.class, 0, addr, conf);
      return new RpcServiceWrapper() {
        @Override
        public String doEcho(String msg) throws Exception {
